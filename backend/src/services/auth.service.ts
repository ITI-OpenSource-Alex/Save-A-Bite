...
const signTokens = (user: IUser): { accessToken: string; refreshToken: string } => {
    ...
    const accessToken = jwt.sign(accessPayload, env.JWT.SECRET as string, { expiresIn: env.JWT.EXPIRE as any });
    const refreshToken = jwt.sign(refreshPayload, env.JWT.REFRESH_SECRET as string, { expiresIn: env.JWT.REFRESH_EXPIRE as any });
...
export class AuthService {
    ...
    async refreshToken(refreshToken: string): Promise<{ accessToken: string }> {
        try {
            ...
            const accessToken = jwt.sign(payload, env.JWT.SECRET as string, { expiresIn: env.JWT.EXPIRE as any });
    ...
    async resetPassword(data: ResetPasswordDto): Promise<void> {
        ...
        const user = await User.findOne({ email, otpExpiresAt: { $gt: new Date() } });
