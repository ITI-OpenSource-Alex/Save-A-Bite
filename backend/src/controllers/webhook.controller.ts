          ...
          if (order) {
            ...
            if (order.promocode) {
              try {
                ...
                if (promo) {
                  logger.info(
                    `Stripe Webhook: Incremented usage for promo code ${order.promocode}`
                  );
                } else {
                  logger.error(
                    `Stripe Webhook: Promo code ${order.promocode} not found in DB`,
                    error
                  );
                }
              } catch (promoError) {
                logger.error(
                  `Stripe Webhook: Failed to increment usage for promo code ${order.promocode}. Error: ${promoError}`,
                  error
                );
              }
            }
